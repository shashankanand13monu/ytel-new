...
class _ContactPageState extends State<ContactPage> {
  ...
  getSearchContact(Map<String, dynamic> body) async {
    ...
    if (isInternetOn) {
      ...
      if (contactController.searchContactModel != null) {
        listOfCheckBox = List.generate(
            contactController.searchContactModel!.payload!.length,
            (index) => false);
      } else {
  ...
  @override
  void initState() {
    getSearchContact({"from": fromPage, "size": toPage, "statements": []});
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ContactController>(
      builder: (_) => Scaffold(
        ...
        body: contactController.searchContactModel != null
            ? _body()
            : const SizedBox(),
      ),
    );
        ...
        child: Row(
          ...
          children: [
            ...
            CustomPageButton(
              onTap: () {
                if (fromPage > 0) {
                  getSearchContact({"from": 0, "size": 100, "statements": []});
                }
                setState(() {
                  fromPage = 0;
                  toPage = 100;
            ...
            CustomPageButton(
              onTap: () {
                if (toPage < contactController.searchContactModel!.count!) {
                  setState(() {
                    fromPage =
                        contactController.searchContactModel!.count! - 100;
  ...
  Widget _body() => ListView.builder(
        ...
        itemBuilder: (context, index) => ExpansionTile(
          ...
          title: Text(
            contactController
                .searchContactModel!.payload![index].extData!.phonenumber1!
                .toString(),
            style: const TextStyle(
              color: Colors.black,
              fontSize: 18,
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    contactController
                        .searchContactModel!.payload![index].created!,
                    style: const TextStyle(
                      color: Colors.black,
                      fontSize: 15,
                      fontWeight: FontWeight.w500,
